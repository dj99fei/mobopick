...
public class DownloadInfo {
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            info.now = System.currentTimeMillis();
            synchronized (this) {
    ...
    public String getSpeed() {
        long deltaTime = Math.max((now - lastNotify) / 1024, 1);
        if ( deltaTime < 2) {
            return speed;
        }
        long deltaSize = (mCurrentBytes - notifyedBytes) / 1000;
        float speed  = Float.valueOf(deltaSize) / deltaTime;
        DecimalFormat format = new DecimalFormat("0.0");
        lastNotify = now;
        notifyedBytes = mCurrentBytes;
        this.speed = (format.format(speed) + " Kb/s");
        return this.speed;
    }
